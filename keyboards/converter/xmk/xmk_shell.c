...
void xmk_shell(char *line) {
  dprintfmt("xmk_shell: line: '%s'\n", line);
  if (strncmp(line, XMK_SHELL_KEY, strlen(XMK_SHELL_KEY)) == 0) {
    dprintfmt("xmk_shell: XMK_SHELL_KEY\n");
    if (strncmp(line + strlen(XMK_SHELL_KEY), XMK_SHELL_KEY_PRESS, strlen(XMK_SHELL_KEY_PRESS)) == 0) {
      ...
      dprintfmt("xmk_shell: XMK_SHELL_KEY_PRESS: key: %u\n", key);
      ...
      dprintfmt("xmk_shell: XMK_SHELL_KEY_RELEASE: key: %u\n", key);
      xmk_matrix_key(false, key);
    }
  } else if (strcmp(line, XMK_SHELL_BOOT) == 0) {
    dprintfmt("xmk_shell: XMK_SHELL_BOOT\n");
    reset_keyboard();
  } else if (strcmp(line, XMK_SHELL_RESET) == 0) {
    dprintfmt("xmk_shell: XMK_SHELL_RESET\n");
    soft_reset_keyboard();
  }
}
...
void virtser_recv(const uint8_t ch) {
  ...
  if (ch == '\r') {
    dprintfmt("virtser_recv: i: %3u, ch: %3u '\\r' \n", line_index, ch);
    line[line_index] = '\0';
    xmk_shell(line);
    line_index = 0;
  } else if (ch == '\n') {
    dprintfmt("virtser_recv: i: %3u, ch: %3u '\\n' \n", line_index, ch);
  } else {
    dprintfmt("virtser_recv: i: %3u, ch: %3u '%c'\n", line_index, ch, ch);
